  ...
  return dedent(
    """
        :root {
          ...
          --prw-purple: #754dac;
          --prw-yellow: #c0872a;
          --prw-font-sans: 'DM Sans', system-ui, sans-serif;
          --prw-font-mono: 'Roboto Mono', ui-monospace, monospace;
        }
        * { box-sizing: border-box; }
        body { margin: 0; min-height: 100vh; background: var(--prw-bg); color: var(--prw-text); font-family: var(--prw-font-sans); }
        a { color: var(--prw-text); text-decoration-color: var(--prw-accent); text-underline-offset: 3px; }
        button, input { font: inherit; }
        .d3-walkthrough-shell { min-height: 100vh; display: grid; grid-template-rows: auto 1fr; }
        .d3-walkthrough-header { display: grid; gap: 10px; padding: 28px 32px 20px; border-bottom: 1px solid var(--prw-border); background: linear-gradient(180deg, #1e1e1d, #121212); }
        .d3-kicker { color: var(--prw-accent); font-family: var(--prw-font-mono); font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; }
        .d3-walkthrough-header h1 { margin: 0; max-width: 1080px; font-size: clamp(34px, 5vw, 72px); line-height: 0.95; letter-spacing: -0.04em; }
        .d3-meta-row { display: flex; flex-wrap: wrap; gap: 8px; color: var(--prw-muted); font-family: var(--prw-font-mono); font-size: 12px; }
        .d3-summary { max-width: 920px; margin: 0; color: var(--prw-muted); font-size: 17px; line-height: 1.45; }
        .d3-canvas-layout { min-height: 0; display: grid; grid-template-columns: 310px minmax(560px, 1fr) 390px; gap: 0; }
        .d3-control-panel, .d3-detail-panel { min-height: 0; overflow: auto; background: var(--prw-panel); border-right: 1px solid var(--prw-border); padding: 18px; }
        .d3-detail-panel { border-right: 0; border-left: 1px solid var(--prw-border); }
        .d3-panel-title { margin: 0 0 12px; font-size: 12px; color: var(--prw-muted); font-family: var(--prw-font-mono); letter-spacing: 0.08em; text-transform: uppercase; }
        .d3-control-stack { display: grid; gap: 10px; margin-bottom: 18px; }
        .d3-control-button, .d3-graph-toggle { border: 1px solid var(--prw-border); background: var(--prw-panel-2); color: var(--prw-text); border-radius: 10px; padding: 10px 12px; cursor: pointer; text-align: left; }
        .d3-control-button:hover, .d3-graph-toggle:hover, .d3-control-button:focus, .d3-graph-toggle:focus { border-color: var(--prw-accent); outline: none; }
        .d3-graph-toggle[aria-pressed="true"] { border-color: var(--graph-color, var(--prw-accent)); box-shadow: inset 3px 0 0 var(--graph-color, var(--prw-accent)); }
        .d3-tour-card { border: 1px solid var(--prw-border); background: #121212; border-radius: 12px; padding: 12px; margin-bottom: 14px; }
        .d3-tour-step-label { color: var(--prw-accent); font-family: var(--prw-font-mono); font-size: 11px; letter-spacing: 0.08em; text-transform: uppercase; }
        .d3-tour-title { margin: 6px 0; font-size: 18px; line-height: 1.15; }
        .d3-tour-body { color: var(--prw-muted); line-height: 1.4; margin: 0; }
        .d3-search { width: 100%; border: 1px solid var(--prw-border); background: #121212; color: var(--prw-text); border-radius: 10px; padding: 10px 12px; }
        .d3-search:focus { border-color: var(--prw-accent); outline: none; }
        .d3-help { color: var(--prw-dim); font-family: var(--prw-font-mono); font-size: 11px; line-height: 1.5; }
        ...
        .d3-canvas-error { position: absolute; inset: 18px; display: none; place-items: center; border: 1px solid var(--prw-border); background: #1e1e1df2; color: var(--prw-text); padding: 24px; z-index: 2; }
        body.d3-canvas-error .d3-canvas-error { display: grid; }
        .d3-graph-title { fill: #faf9f6; opacity: 0.36; font-family: var(--prw-font-mono); font-size: 13px; letter-spacing: 0.08em; text-transform: uppercase; }
        ...
        .d3-edge text { fill: #cccbc8; font-family: var(--prw-font-mono); font-size: 11px; paint-order: stroke; stroke: #121212; stroke-width: 4px; stroke-linejoin: round; }
        .d3-node { cursor: pointer; }
        .d3-node rect { fill: #1e1e1d; stroke: var(--node-color, var(--prw-accent)); stroke-width: 2; filter: drop-shadow(0 10px 24px #00000066); }
        .d3-node.is-selected rect { stroke: var(--prw-accent); stroke-width: 4; }
        .d3-node.is-tour-node rect { stroke: var(--prw-accent); stroke-width: 4; filter: drop-shadow(0 0 18px #a4378788); }
        .d3-node.is-dimmed, .d3-edge.is-dimmed { opacity: 0.18; }
        .d3-node-title { fill: #faf9f6; font-family: var(--prw-font-sans); font-size: 15px; font-weight: 700; pointer-events: none; }
        .d3-node-kind { fill: #b4b4b2; font-family: var(--prw-font-mono); font-size: 10px; letter-spacing: 0.08em; text-transform: uppercase; pointer-events: none; }
        .d3-node-summary { fill: #cccbc8; font-family: var(--prw-font-sans); font-size: 12px; pointer-events: none; }
        .d3-detail-title { margin: 0 0 6px; font-size: 24px; line-height: 1.1; }
        .d3-detail-kind { color: var(--prw-accent); font-family: var(--prw-font-mono); font-size: 11px; letter-spacing: 0.08em; text-transform: uppercase; }
        .d3-detail-summary { color: var(--prw-muted); line-height: 1.45; }
        .d3-detail-section { margin-top: 18px; }
        .d3-detail-section h3 { margin: 0 0 8px; color: var(--prw-muted); font-family: var(--prw-font-mono); font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; }
        .d3-detail-list { display: grid; gap: 8px; margin: 0; padding: 0; list-style: none; }
        .d3-detail-list li { border: 1px solid var(--prw-border); background: #121212; border-radius: 10px; padding: 10px; color: var(--prw-muted); line-height: 1.35; }
        .d3-file-link { display: block; overflow-wrap: anywhere; color: var(--prw-text); font-family: var(--prw-font-mono); font-size: 12px; }
        .d3-comment-author { display: block; color: var(--prw-accent); font-family: var(--prw-font-mono); font-size: 11px; margin-bottom: 4px; }
        .d3-empty { color: var(--prw-dim); }
        @media (max-width: 1180px) { .d3-canvas-layout { grid-template-columns: 1fr; grid-template-rows: auto minmax(680px, 1fr) auto; } .d3-control-panel, .d3-detail-panel { border: 0; border-bottom: 1px solid var(--prw-border); max-height: 360px; } }
        """
  ).strip()
...
def sample_data() -> dict:
  return {
    "meta": {
      "title": "Sample PR D3 walkthrough",
      "prUrl": "",
      "baseRef": "main",
      "headRef": "feature",
      "summary": "Replace this sample graph with PR-specific guided graph tours.",
    },
    "graphs": [
      {
        "id": "system-overview",
        ...
        "color": "#c0872a",
        "summary": "Major touched components.",
        "nodes": [
          {
            "id": "surface",
            "title": "User-facing surface",
            "kind": "overview card",
            "x": -220,
            "y": -80,
            "width": 360,
            "height": 220,
            "summaryLines": 7,
            "summary": "Use a full paragraph here to define the surface, what code owns it, and why a reviewer needs that concept before reading the PR.",
            "details": ["Explain the stable component."],
            "files": [],
            "comments": [],
            "links": [],
          },
          {
            ...
            "height": 220,
            "summaryLines": 7,
            "summary": "Use another full paragraph for the next essential concept. If a concept is not needed to understand the review surface, leave it out.",
            "details": ["Explain what this component owns."],
            "files": [],
            "comments": [],
            "links": [],
          },
        ...
        "tour": [
          {
            ...
            "body": "The system overview starts with the smallest useful orientation concept.",
          },
          {
            "nodeId": "component",
            "title": "Name the owner",
            "body": "Then identify the state or action owner a reviewer needs to know.",
          },
        ],
      },
      {
        ...
        "nodes": [
          {
            ...
            "x": -260,
            "y": -80,
            "summary": "PR intent enters the system.",
            "details": ["Start with the PR description or commit messages."],
            "files": [],
            "comments": [],
            "links": [],
          },
          {
            ...
            "x": 80,
            "y": 20,
            "summary": "State owner carries the change.",
            "details": ["Explain the data owner."],
            "files": [],
            "comments": [],
            "links": [],
          },
        ...
        "tour": [
          {
            ...
            "body": "The data-flow graph begins with the PR intent.",
          },
          {
            "nodeId": "state",
            "title": "Follow state",
            "body": "Then inspect where state is owned.",
          },
        ],
      },
      {
        ...
        "nodes": [
          {
            ...
            "x": -220,
            "y": -60,
            "summary": "Changed entry point.",
            "details": ["Start at the high-level code seam."],
            "files": [],
            "comments": [],
            "links": [],
          },
          {
            ...
            "x": 160,
            "y": 70,
            "summary": "Lower-level dependency.",
            "details": ["Inspect the dependency."],
            "files": [],
            "comments": [],
            "links": [],
          },
        ...
        "tour": [
          {
            "nodeId": "entry",
            "title": "Start at entry",
            "body": "Begin with the high-level code seam.",
          },
          {"nodeId": "leaf", "title": "Drill down", "body": "Then move to the leaf dependency."},
        ],
      },
      {
        "id": "user-action",
        "label": "User action graph",
        "color": "#754dac",
        "summary": "How the user moves.",
        "nodes": [
          {
            "id": "surface",
            "title": "Surface",
            ...
            "x": -240,
            "y": -70,
            "summary": "Where the user starts.",
            "details": ["Explain the user-facing surface."],
            "files": [],
            "comments": [],
            "links": [],
          },
          {
            ...
            "details": ["Explain the visible result."],
            "files": [],
            "comments": [],
            "links": [],
          },
        ...
        "tour": [
          {"nodeId": "surface", "title": "Start at surface", "body": "Begin where the user acts."},
          {
            "nodeId": "feedback",
            "title": "End at feedback",
            "body": "End with what the user sees.",
          },
        ],
      },
    ],
...
def main() -> int:
  parser = argparse.ArgumentParser(
    description="Emit reusable D3 PR walkthrough graph tour snippets."
  )
  group = parser.add_mutually_exclusive_group(required=True)
  group.add_argument("--css", action="store_true", help="Print D3 graph CSS.")
  group.add_argument(
    "--runtime", action="store_true", help="Print pinned-CDN D3 runtime and graph renderer."
  )
  group.add_argument(
    "--template", action="store_true", help="Print a complete HTML template from graph JSON."
  )
  group.add_argument("--sample-data", action="store_true", help="Print sample graph JSON.")
  parser.add_argument(
    "--data", type=Path, help="Graph JSON file for --template. If omitted, sample data is used."
  )
