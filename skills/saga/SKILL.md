---
name: saga
description: Use only when the developer explicitly asks to run a saga for an already-approved implementation checklist, with phrases like "execute this as a saga" or "saga this checklist." A saga coordinates checkbox execution, tracks progress, runs verification loops, integrates results, and escalates to the human only for true blockers.
