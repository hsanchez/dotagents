...
implicit initialization; compatibility providers require explicit selection
or an existing lockfile.

| Provider | Lifecycle | Default |
| --- | --- | --- |
| Claude | Active | Yes |
| Codex | Active | Yes |
| Copilot CLI | Active | Yes |
| Antigravity CLI (`agy`) | Active; primary Google CLI | Yes |
| Gemini CLI | Compatibility for Enterprise/API-key users | No |

Google transitioned its individual terminal experience from Gemini CLI to
Antigravity CLI on June 18, 2026. Gemini CLI remains available to Enterprise
customers and API-key users, so dotagents preserves it as an explicitly
selectable provider. Existing lockfiles continue syncing Gemini without
migration or removal. See Google's
[transition announcement](https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/)
and [Gemini CLI release notes](https://github.com/google-gemini/gemini-cli/blob/main/docs/changelogs/index.md).

The provider keys remain separate. Their configuration roots, hook contracts,
tool names, and release lifecycles differ. Antigravity reads the shared
`AGENTS.md` and `.agents/skills` paths already generated by dotagents; Gemini's
legacy `settings.json` is not copied into the agy provider. Google's
[migration guide](https://www.antigravity.google/docs/gcli-migration)
documents the compatible context and skill paths.
...
| `scoped` | `acceptEdits` | `never` / `workspace-write` | Auto-allow native edits whose targets are all inside the repo; preserve other approvals | Allow known reads and native workspace edits at the hook gate; deny every other tool |
...
Claude applies a native deny ceiling at every level. Copilot and agy use a
shared hook classifier that denies recognized direct invocations of `git
push`, `git reset --hard`, forced `git clean`, `sudo`, and the broader
destructive-Git forms covered by `git-guardrails`. Shell indirection such as
`sh -c`, `bash -c`, `zsh -c`, and equivalent common command-shell forms is
inspected recursively. General-purpose interpreters, scripts, encoded
commands, and other unclassified execution remain under the provider's
normal permission flow. Copilot hook timeouts also return control to that
flow.
...
Gemini CLI remains a compatibility provider separate from agy and has no
autonomy fragments. Selecting `gemini` installs only Gemini assets; select
...
denies them. Copilot `apply_patch` is auto-allowed only when every parsed
target resolves inside the repository; malformed or unscoped patches return
to Copilot's permission flow. Repo hooks are auditable policy for agent
execution, not a security boundary against shell indirection or a user who
can edit or disable repository configuration. The shell-wrapper inspection
closes common accidental and straightforward bypasses; it does not make
lexical command classification an unbypassable sandbox.
